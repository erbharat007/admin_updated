...
{
	case "updatesave":
		$arrayData = array($_REQUEST['full-year-calendar'], 'south');
		$_SESSION['msg'] = $objHoliday->updateHolidays($arrayData);
	break;
}

$rsHolidays = $objHoliday->getHolidaysOfYear('all', 'south');
$holidaysList = array();
...
function updateReason(editableObj, id) 
{
	$.ajax({
		...
		dataType: 'json',
		type: "POST",
		data: {'action': 'updateHolidayReason', 'region': 'south', 'reasonOfHoliday' : editableObj.innerHTML, 'id': id},
		beforeSend: function() 
		{
...
<div class="widget-content">
	<form name="frm_dept" method="post" enctype="multipart/form-data" action="index.php?Option=Setup&SubOption=holidayCalendarSouth&ofaction=updatesave<?php echo $additional_link_redirect;?>" class="form-horizontal col-md-6" role="form" onsubmit="return checkData();";>
	...
	<?php
	$dsqle = $objHoliday->getHolidaysOfYear('currentYear', 'south');
	$total_pages = $GLOBALS['obj_db']->num_rows($dsqle);
	?>
	
	<div class="col-md-12" style="clear:both;">
		<form name="search_form" method="post" action="index.php?Option=Setup&SubOption=holidayCalendarSouth">
			<input type="search" required name="search_text" value="<?php echo $_REQUEST['search_text']; ?>">
			<input type="submit" class="btn btn-info" value="Search">
			<input type="button" onclick="window.location.href='index.php?Option=Setup&SubOption=holidayCalendarSouth';" class="btn btn-danger" value="Cancel" />				
		</form>
	</div>	
	...
	<form action="index.php?Option=Setup&SubOption=holidayCalendarSouth" method="post" name="frmUpdate" id="frmUpdate">
		<div class="table-responsive">
			<table class="table table-bordered table-hover table-striped">
				...
				$targetpage = "index.php?Option=Setup&SubOption=holidayCalendarSouth";

				$contentrow = $objHoliday->getHolidaysOfYear('currentYear', 'south', $start, $end);
