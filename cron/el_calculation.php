...
if($GLOBALS['obj_db']->num_rows($rsEmp) > 0)
{
	...
	while($rowEmp = $GLOBALS['obj_db']->fetch_array($rsEmp))
	{
		...
		*/
		...
		$sqlUpdate = "UPDATE leave_balance SET opening_balance = opening_balance+".$elToBeAdded.", last_updated = '".$currentDateObj->format('Y-m-d H:i:s')."', remaining_countable_days = ".$remainingCountableDays.", updated_by = 'CRON', updated_by_user_type = 'CRON' WHERE emp_id = '".$rowEmp['emp_id']."' AND leave_type_id = '".$leaveTypeId."' AND year = YEAR(CURDATE()) ";
...
$file = fopen("logs/log_".$currentDateObj->format("d_m_Y").'.txt', "w");
if($file)
{
